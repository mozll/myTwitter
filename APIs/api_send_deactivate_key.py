...
@post('/send-deactivate-key')
def _():
    try:
        user_deactivation_key = str(uuid.uuid4()).replace("-","")
        ...
        rows_affected = db.execute(f"""
            UPDATE users
            SET user_deactivation_key = ?
            WHERE user_email = ?
            """,(user_deactivation_key, user_obj.get("user_email"))).rowcount
        ...
        and use the deactivation key: {user_deactivation_key}
        """
        html = f"""\
        <html>
        <body>
